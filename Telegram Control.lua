   ...
   if upd_res then -- если получили json
      if thisScript().version ~= upd_res.version then -- если версия не сходится с json
         if upd_res.url then -- если есть ссылка на файл
            downloadUpdate(upd_res.url) -- качаем обновление
         else
            msg("Возникла ошибка при загрузке обновления!")
         end
      else
         msg("У вас акутальная версия!")
      end

   else
      msg("Возникла ошибка при проверке обновлений!")
   end
end
